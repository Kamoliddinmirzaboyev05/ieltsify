...
import { Upload as UploadIcon, Plus, Trash2, FileText } from 'lucide-react';
...
const ReadingPassageManager: React.FC = () => {
  ...
  const [passages, setPassages] = useState(readingPassageManager.getAll());
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      readingPassageManager.add({
        title,
        htmlContent,
        imageUrl: imageUrl || undefined,
      });

      message.success('Reading passage qo\'shildi!');
      setTitle('');
      setHtmlFile(null);
      setImageUrl('');
  ...
  const handleDelete = (id: string) => {
    Modal.confirm({
      ...
      onOk: () => {
        readingPassageManager.delete(id);
        setPassages(readingPassageManager.getAll());
    ...
    <div style={{ paddingBottom: '40px' }}>
      ...
      <div style={{ marginBottom: '32px' }}>
        <Title level={1} style={{ margin: 0, fontSize: isMobile ? '24px' : '32px', fontWeight: '800' }}>
          📖 Reading Passage Manager
        </Title>
        <Text type="secondary" style={{ fontSize: isMobile ? '14px' : '16px' }}>
          Reading mock test uchun HTML fayllar yuklang
        </Text>
      </div>
      ...
      <Card style={{ borderRadius: '16px' }}>
        <Title level={4} style={{ marginBottom: '16px' }}>
          Mavjud Passagelar ({passages.length})
        </Title>
        {passages.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 20px' }}>
            <FileText size={48} style={{ color: '#94a3b8', marginBottom: '16px' }} />
            <Text type="secondary">Hali passagelar yo'q</Text>
          </div>
        ) : (
            ...
            {passages.map((passage) => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
                  <div style={{ flex: 1 }}>
                    <Text strong style={{ fontSize: '16px' }}>{passage.title}</Text>
                    <div style={{ marginTop: '4px' }}>
                      {passage.imageUrl && <Tag>🖼️ Rasm bor</Tag>}
                      <Text type="secondary" style={{ fontSize: '12px', marginLeft: '8px' }}>
                        {new Date(passage.uploadDate).toLocaleDateString()}
                      </Text>
                    </div>
                    {passage.imageUrl && (
                      <div style={{ marginTop: '12px' }}>
...
export default ReadingPassageManager;
