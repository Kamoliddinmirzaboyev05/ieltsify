...
type ExamPhase = 'intro' | 'part1' | 'part2-prep' | 'part2-speak' | 'part3' | 'complete';
...
const SpeakingPageNew: React.FC = () => {
  ...
  const startRecording = () => {
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
      message.error("Your browser doesn't support speech recognition. Please use Chrome/Edge.");
      return;
    }
      ...
      .then(() => {
        ...
        reco.onerror = (event: any) => {
          ...
          if (event.error === 'not-allowed') {
            message.error("Microphone access blocked. Please allow permissions.");
  ...
  const handleNextQuestion = () => {
    ...
    if (examPhase === 'part1') {
      ...
      const currentQuestion = selectedTest.part3.questions[currentQuestionIndex];
  ...
  const analyzeExam = async () => {
    ...
    try {
      ...
      const allAnswers = [
        ...examSession.part1Answers.map(a => `Q: ${a.question}\nA: ${a.transcript}`),
        examSession.part2Answer ? `Part 2: ${examSession.part2Answer.transcript}` : '',
        ...examSession.part3Answers.map(a => `Q: ${a.question}\nA: ${a.transcript}`),
      ].join('\n\n');
...
Provide a comprehensive analysis in JSON format:
{
  ...
  "vocabulary_suggestions": ["word 1", "word 2", "word 3", "word 4", "word 5"]
      ...
      const result = await evaluateSpeaking('Full IELTS Speaking Test', analysisPrompt);
          ...
          <Col xs={12} sm={12} lg={6} key={i}>
            <Card style={{ borderRadius: '16px', border: 'none', boxShadow: '0 4px 15px rgba(0,0,0,0.02)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <Text type="secondary" strong style={{ fontSize: '10px', display: 'block' }}>{stat.label}</Text>
                  <Title level={2} style={{ margin: '4px 0 0 0', fontSize: isMobile ? '20px' : '28px' }}>{stat.value}</Title>
                </div>
  ...
  const renderExam = () => {
    ...
    if (examPhase === 'intro') {
      return (
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <Card style={{ borderRadius: '32px', padding: '40px', textAlign: 'center' }}>
            <Avatar size={100} icon={<User size={50} />} style={{ backgroundColor: '#f3e8ff', color: '#6B46C1', marginBottom: '24px' }} />
            <Title level={2}>IELTS Speaking Test</Title>
            <Title level={4} type="secondary">{selectedTest.title}</Title>
            <Divider />
            <div style={{ textAlign: 'left', marginBottom: '32px' }}>
              <Paragraph><strong>Part 1:</strong> Introduction & General Topics (4-5 minutes)</Paragraph>
              <Paragraph><strong>Part 2:</strong> Individual Long Turn with Cue Card (3-4 minutes)</Paragraph>
              <Paragraph><strong>Part 3:</strong> Two-way Discussion (4-5 minutes)</Paragraph>
            </div>
            <Button type="primary" size="large" onClick={handleNextQuestion} icon={<Play size={20} />}>
              Begin Test
            </Button>
          </Card>
        </div>
      ...
      return (
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Tag color="blue" style={{ fontSize: '14px', padding: '4px 16px' }}>Part 1: {currentTopic.topic}</Tag>
            <Text type="secondary">Question {currentQuestionIndex + 1} of {currentTopic.questions.length}</Text>
          </div>
          <Progress percent={Math.round(progress)} showInfo={false} strokeColor="#6B46C1" style={{ marginBottom: '24px' }} />
          
          <Card style={{ borderRadius: '32px', padding: '60px 20px', textAlign: 'center', marginBottom: '24px' }}>
            <Avatar size={120} icon={<User size={60} />} style={{ backgroundColor: '#f3e8ff', color: '#6B46C1', marginBottom: '32px' }} />
            <Title level={3} style={{ minHeight: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              "{currentQuestion}"
            </Title>
            ...
            <div style={{ marginTop: '40px' }}>
              <div className={recordingState === 'recording' ? 'recording-indicator' : ''} style={{ width: '120px', height: '120px', margin: '0 auto' }}>
                ...
                >
                  {recordingState === 'recording' ? <MicOff size={48} /> : <Mic size={48} />}
                </Button>
              </div>
              <Text strong style={{ display: 'block', marginTop: '24px', fontSize: '16px', color: recordingState === 'recording' ? '#ef4444' : '#64748b' }}>
                {recordingState === 'recording' ? `Recording... ${formatTime(timer)}` : 'Click to answer'}
              </Text>
              {transcript && (
                <div style={{ marginTop: '20px', padding: '16px', backgroundColor: '#f8fafc', borderRadius: '12px', maxWidth: '600px', margin: '20px auto' }}>
                  <Text italic style={{ fontSize: '14px', color: '#475569' }}>{transcript}</Text>
                </div>
              )}
            </div>
          </Card>
          ...
          <div style={{ textAlign: 'center' }}>
            <Button size="large" onClick={handleNextQuestion} disabled={!transcript.trim()}>
              Next Question
            </Button>
          </div>
        </div>
    ...
    if (examPhase === 'part2-prep') {
      return (
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <Tag color="green" style={{ fontSize: '14px', padding: '4px 16px', marginBottom: '24px' }}>Part 2: Preparation Time</Tag>
          
          <Card style={{ borderRadius: '24px', padding: '40px', marginBottom: '24px' }}>
            ...
            <Divider />
            <div style={{ textAlign: 'center' }}>
              <Title level={1} style={{ color: '#6B46C1', fontSize: '72px', margin: '20px 0' }}>
                {Math.max(0, 60 - timer)}
              </Title>
              <Text type="secondary" style={{ fontSize: '18px' }}>seconds remaining to prepare</Text>
              <Paragraph type="secondary" style={{ marginTop: '16px' }}>
                Take notes and prepare your answer. You will have 2 minutes to speak.
              </Paragraph>
            </div>
          </Card>
    ...
    if (examPhase === 'part2-speak') {
      ...
      return (
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <Tag color="green" style={{ fontSize: '14px', padding: '4px 16px', marginBottom: '24px' }}>Part 2: Speaking</Tag>
          
          <Card style={{ borderRadius: '32px', padding: '40px', textAlign: 'center', marginBottom: '24px' }}>
            <Title level={4} style={{ marginBottom: '32px' }}>{selectedTest.part2.cueCard}</Title>
            
            <div style={{ marginBottom: '32px' }}>
              <Title level={1} style={{ color: timer >= timeLimit ? '#ef4444' : '#6B46C1', fontSize: '64px', margin: 0 }}>
            ...
            <Text strong style={{ display: 'block', marginTop: '24px', fontSize: '16px' }}>
              {recordingState === 'recording' ? 'Recording...' : 'Click to start speaking'}
            </Text>
          </Card>
    ...
    if (examPhase === 'part3') {
      const currentQuestion = selectedTest.part3.questions[currentQuestionIndex];
      const progress = ((currentQuestionIndex + 1) / selectedTest.part3.questions.length) * 100;

      return (
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Tag color="purple" style={{ fontSize: '14px', padding: '4px 16px' }}>Part 3: {selectedTest.part3.topic}</Tag>
            <Text type="secondary">Question {currentQuestionIndex + 1} of {selectedTest.part3.questions.length}</Text>
          </div>
          <Progress percent={Math.round(progress)} showInfo={false} strokeColor="#a855f7" style={{ marginBottom: '24px' }} />
          
          <Card style={{ borderRadius: '32px', padding: '60px 20px', textAlign: 'center', marginBottom: '24px' }}>
            <Avatar size={120} icon={<User size={60} />} style={{ backgroundColor: '#faf5ff', color: '#a855f7', marginBottom: '32px' }} />
            <Title level={3} style={{ minHeight: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              "{currentQuestion}"
            </Title>
              ...
              <div className={recordingState === 'recording' ? 'recording-indicator' : ''} style={{ width: '120px', height: '120px', margin: '0 auto' }}>
                <Button
                  ...
                  style={{
                    ...
                    height: '120px',
                    borderRadius: '50%',
                    backgroundColor: recordingState === 'recording' ? '#ef4444' : '#a855f7',
                    color: 'white',
                    border: 'none',
                  }}
                >
          ...
          <div style={{ textAlign: 'center' }}>
            <Button size="large" onClick={handleNextQuestion} disabled={!transcript.trim()}>
              {currentQuestionIndex < selectedTest.part3.questions.length - 1 ? 'Next Question' : 'Finish Test'}
            </Button>
          </div>
        </div>
      );
    ...
    if (examPhase === 'complete') {
      if (isAnalyzing) {
        return (
          <div style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center', padding: '100px 20px' }}>
            <div style={{ fontSize: '64px', marginBottom: '24px' }}>🤖</div>
            <Title level={3}>Analyzing Your Performance...</Title>
            <Paragraph type="secondary">Our AI examiner is reviewing your answers and preparing detailed feedback.</Paragraph>
            <Progress percent={100} status="active" showInfo={false} strokeColor="#6B46C1" style={{ marginTop: '32px' }} />
          </div>
        );
      }
      ...
      if (!finalAnalysis) {
        return (
          <div style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center', padding: '100px 20px' }}>
            ...
            <Paragraph>Please check your API key configuration and try again.</Paragraph>
            <Button onClick={() => setPageState('listing')}>Back to Tests</Button>
          </div>
        );
      }
      ...
      return (
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <Button icon={<ArrowLeft size={18} />} onClick={() => setPageState('listing')} style={{ marginBottom: '24px' }}>
            Back to Tests
          </Button>

          <Card style={{ borderRadius: '32px', padding: '40px', marginBottom: '24px', textAlign: 'center' }}>
            <div style={{ width: '140px', height: '140px', borderRadius: '40px', backgroundColor: '#f3e8ff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', transform: 'rotate(5deg)' }}>
              <Award size={32} color="#6B46C1" />
              <Title level={1} style={{ margin: 0, color: '#6B46C1', fontSize: '56px' }}>{finalAnalysis.overall_band}</Title>
              <Text strong style={{ fontSize: '10px', color: '#6B46C1', letterSpacing: '1px' }}>OVERALL BAND</Text>
            </div>
            <Title level={2}>Test Complete!</Title>
            <Text type="secondary">Here's your comprehensive IELTS Speaking assessment</Text>
          </Card>

          <Row gutter={[24, 24]}>
            <Col xs={24} md={12}>
              <Card title="Band Scores Breakdown" style={{ borderRadius: '24px', height: '100%' }}>
                <Space direction="vertical" size="large" style={{ width: '100%' }}>
                  {[
                    { label: 'Fluency & Coherence', score: finalAnalysis.fluency_coherence, color: '#3b82f6' },
                    { label: 'Lexical Resource', score: finalAnalysis.lexical_resource, color: '#10b981' },
                    { label: 'Grammatical Range', score: finalAnalysis.grammatical_range, color: '#f59e0b' },
                    { label: 'Pronunciation', score: finalAnalysis.pronunciation, color: '#ef4444' },
                  ].map((item, idx) => (
                    <div key={idx}>
                      ...
                      <Progress percent={(item.score / 9) * 100} showInfo={false} strokeColor={item.color} />
                    </div>
                  ))}
                </Space>
              </Card>
            ...
            <Col xs={24} md={12}>
              <Card title="Vocabulary Suggestions" style={{ borderRadius: '24px', height: '100%', backgroundColor: '#eff6ff' }}>
                <Space wrap size={[8, 12]}>
                  {finalAnalysis.vocabulary_suggestions.map((word: string, idx: number) => (
                    <Tag key={idx} color="blue" style={{ borderRadius: '8px', padding: '4px 12px', fontSize: '13px' }}>
                      {word}
                    </Tag>
                  ))}
                </Space>
          ...
          <div style={{ textAlign: 'center', marginTop: '40px' }}>
            <Button type="primary" size="large" onClick={() => setPageState('listing')}>
              Take Another Test
            </Button>
          </div>
        </div>
