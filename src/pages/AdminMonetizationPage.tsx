  ...
  const columns = [
      ...
      render: (record: any) => (
        <Space direction="vertical" size={0}>
          ...
          <Text type="secondary" style={{ fontSize: "12px" }}>
            Balans:{" "}
            {Array.isArray(record.user_coins)
              ? record.user_coins[0]?.balance
              : record.user_coins?.balance || 0}{" "}
            coin
          </Text>
        </Space>
      ),
    },
    {
      ...
      render: (type: string, record: any) => (
        <Tag color={type === "subscription" ? "purple" : "gold"}>
          {type === "subscription"
            ? `Obuna: ${record.plan_type}`
            : `Coins: ${record.coin_amount}`}
        </Tag>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (status: string) => {
        ...
        return (
          <Tag color={(colors as any)[status]}>{status.toUpperCase()}</Tag>
        );
      },
    },
    {
          ...
          <div style={{ padding: "16px 0" }}>
            <Row gutter={24}>
              <Col span={12}>
                ...
                <Paragraph strong>
                  {selectedRequest.amount.toLocaleString()} so'm
                </Paragraph>
              </Col>
              <Col span={12}>
